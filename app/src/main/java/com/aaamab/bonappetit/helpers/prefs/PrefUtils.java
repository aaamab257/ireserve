package com.aaamab.bonappetit.helpers.prefs;
...
import static com.aaamab.bonappetit.helpers.prefs.PrefKeys.App_BONAPPETIT;



/**
 * Created by Ahmed Ashraf in 27 / 12 / 2020 .
 *
 */
public class PrefUtils {
    ...
    public static void saveOpenStatus(boolean indexlang, Context context) {
        SharedPreferences.Editor editor = context.getSharedPreferences(App_BONAPPETIT, Context.MODE_PRIVATE).edit();
    ...
    public static boolean getOpenStatus(Context context) {
        String lang = "";
        boolean status = context.getSharedPreferences(App_BONAPPETIT, Context.MODE_PRIVATE).getBoolean(Is_First_open, false);
        if (!status) {
            return true;
        }else {
    ...
    public static void saveUserinformation(Context context, LoginData consumerResponse, int status ) {
        SharedPreferences sharedpreferences = context.getSharedPreferences(App_BONAPPETIT, Context.MODE_PRIVATE);
    ...
    public static Boolean getUserformation(Context context) {
        ...
        if (sharedpreferences.getInt(Status_User, User_Singout) != User_Singout){
            ...
            StaticMethods.auth = StaticMethods.userData.getApi_token();
            return  true;
        }else {
            return  false;
        }
